...
// ── Query keys ── (legacy 'stats' prefix still used by repair mutations)
...
async function fetchWeeklyWatch(
  ...
  if (kind === 'shows') {
    // Server-side daily aggregation — one RPC returning one row per watched
    // day instead of every watched episode row (runtime fallback 42min
    // matches ESTIMATED_RUNTIME, applied in SQL).
    ...
    for (const row of data as Array<{ day: string; episode_count: number; total_seconds: number }>) {
      ...
      entry.count += Number(row.episode_count)
      entry.totalSeconds += Number(row.total_seconds)
